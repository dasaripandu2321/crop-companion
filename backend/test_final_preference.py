import sys
import os
os.chdir('C:\\Users\\dasar\\Downloads\\farm-futura-ai-main\\farm-futura-ai-main\\backend')
sys.path.insert(0, '.')

# Force reimport
for mod in list(sys.modules.keys()):
    if 'app' in mod:
        del sys.modules[mod]
    
import app

# Test direct
input_data = {'crop': 'Rice', 'N': 25, 'P': 20, 'K': 20, 'temperature': 25, 'humidity': 60, 'ph': 6.5, 'rainfall': 750}
result = app.mock_predict(input_data)

print("\nTop 15 Crops (With Preference Boost x12):")
print("=" * 60)
for i, crop in enumerate(result['allProbabilities'][:15]):
    is_preferred = crop['name'] in ['Rice', 'Wheat', 'Bajra', 'Maize', 'Chilli', 'Cotton']
    marker = '[PREFERRED]' if is_preferred else ''
    print(f"{i+1:2}. {crop['emoji'] if len(crop['emoji']) < 2 else crop['emoji'][:1]} {crop['name']:<15} {crop['probability']:>7.2f}% {marker}")

print("\nPreferred crops with higher boost:")
for crop in result['allProbabilities']:
    if crop['name'] in ['Rice', 'Wheat', 'Bajra', 'Maize', 'Chilli', 'Cotton']:
        print(f"  {crop['emoji']} {crop['name']}: {crop['probability']:.2f}%")
